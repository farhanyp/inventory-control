...
export function CannotDeleteDialog({ open, onOpenChange, product }: ProductActionDialogProps) {
    const incomingCount = product?.incoming_products_count || 0;
    const salesCount = product?.sales_details_count || 0;
    const totalRelations = incomingCount + salesCount;
            ...
            <DialogContent className="sm:max-w-md p-0 overflow-hidden border-0 shadow-2xl rounded-2xl">
                <div className="flex flex-col items-center text-center p-6 pt-10 pb-8 bg-gradient-to-b from-red-50/50 to-background dark:from-red-950/20 dark:to-background">
                    ...
                    <p className="text-muted-foreground text-sm max-w-sm leading-relaxed mb-6">
                        Produk <strong className="text-foreground font-semibold">"{product?.product_name}"</strong> tidak dapat dihapus karena masih terhubung dengan data transaksi lain yang ada di sistem.
                    </p>
                    ...
                    <div className="w-full bg-card border border-border/50 rounded-xl p-4 mb-8 shadow-sm flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                                <p className="text-sm font-medium text-foreground">Transaksi Terkait</p>
                                <p className="text-xs text-muted-foreground">Masih menggunakan produk ini</p>
                            </div>
                        </div>
                    ...
                    <div className="w-full">
                        ...
                        <p className="text-xs text-muted-foreground mt-4">
                            Hapus transaksi pembelian atau penjualan terkait terlebih dahulu untuk dapat menghapus produk ini.
                        </p>
                    </div>
                </div>
            </DialogContent>
