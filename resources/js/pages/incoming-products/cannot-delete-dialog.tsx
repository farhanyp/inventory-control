...
import type { IncomingProduct } from '@/types/incoming-product';

interface IncomingProductActionDialogProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    incomingProduct: IncomingProduct | null;
}

export function CannotDeleteDialog({ open, onOpenChange, incomingProduct }: IncomingProductActionDialogProps) {
    return (
            ...
            <DialogContent className="sm:max-w-md p-0 overflow-hidden border-0 shadow-2xl rounded-2xl">
                <div className="flex flex-col items-center text-center p-6 pt-10 pb-8 bg-gradient-to-b from-red-50/50 to-background dark:from-red-950/20 dark:to-background">
                    ...
                    <p className="text-muted-foreground text-sm max-w-sm leading-relaxed mb-6">
                        Barang Masuk <strong className="text-foreground font-semibold">"{incomingProduct?.invoice_number}"</strong> tidak dapat dihapus karena stoknya telah terpakai.
                    </p>
                    ...
                    <div className="w-full bg-card border border-border/50 rounded-xl p-4 mb-8 shadow-sm flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                                <p className="text-sm font-medium text-foreground">Stok Digunakan</p>
                                <p className="text-xs text-muted-foreground">Tercatat di data penjualan</p>
                            </div>
                        </div>
                    ...
                    <div className="w-full">
                        ...
                        <p className="text-xs text-muted-foreground mt-4">
                            Sistem mengunci data barang masuk yang stoknya telah digunakan agar laporan tetap valid.
                        </p>
                    </div>
                </div>
            </DialogContent>
